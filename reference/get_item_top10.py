...
def main(querydate, sortopts=None, searchwhat=None):
    ...
    query = ("select last_seen_date, name, msrp, salePrice, storeId, imageUrl, upc, sku, status from deals where upc = "+ searchwhat +" and last_seen_date "+ querydate +" order by salePrice asc limit 150; ")
    #print(query)
    cursor.execute(query)
    
    itemDict = {}
    count = int(0)
    print("{:10} | {:10} | {:12} | {:9} | {:8} | {:9} | {:9} | {}".format("Status", "Date", "UPC", "SKU", "Store ID", "MSRP", "Store Price", "Name"))
    for (date, name, msrp, storePrice, storeId, imageUrl, upc, sku, status) in cursor:
        ...
        count += 1
        print("{7:10} | {6} | {1} | {2:9} | {3:8} | ${4:8,.2f} | ${5:10,.2f} | {0} "
                .format(itemname, upc, sku, storeId, (msrp / 100.00), (storePrice / 100.00), date, status))
    cursor.close()
    cnx.close()
    return itemDict
